...
public class UserController {
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "201", description = "User record successfully provisioned.", content = @Content(schema = @Schema(implementation = RestResponse.class))),
        @ApiResponse(responseCode = "400", description = "Inbound registration data parameters invalid or email collision occurred."),
        @ApiResponse(responseCode = "401", description = "Requesting context lacks valid Bearer credentials."),
        @ApiResponse(responseCode = "403", description = "Account credentials lack sufficient privileges (Admin access required).")
    })
    public ResponseEntity<RestResponse<UserDto>> createUser(@RequestBody UserDto userDto) {
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(
                RestResponse.created("User record successfully provisioned.", createdUser)
        );
    }
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "User list successfully loaded.", content = @Content(schema = @Schema(implementation = RestResponse.class))),
        @ApiResponse(responseCode = "401", description = "Requesting context lacks valid security credentials."),
        @ApiResponse(responseCode = "403", description = "Insufficient role permissions to view the user registry.")
    })
    public ResponseEntity<RestResponse<Iterable<UserDto>>> getAllUsers() {
        ...
        return ResponseEntity.ok(
                RestResponse.success("User list successfully loaded.", users)
        );
    }

    @GetMapping("/email/{email}")
    @PreAuthorize("hasRole('USER') or #email == authentication.principal.username")
    @Operation(
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Target user account isolated and returned.", content = @Content(schema = @Schema(implementation = RestResponse.class))),
        @ApiResponse(responseCode = "401", description = "Requesting context lacks authentication clearance."),
        @ApiResponse(responseCode = "44", description = "No user found matching the requested email identifier.")
    })
    public ResponseEntity<RestResponse<UserDto>> getUserByEmail(
        ...
        return ResponseEntity.ok(
                RestResponse.success("User profile retrieved successfully.", userdto)
        );
    }
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "User changes successfully updated and saved.", content = @Content(schema = @Schema(implementation = RestResponse.class))),
        @ApiResponse(responseCode = "400", description = "Provided request body contains invalid data states or types."),
        @ApiResponse(responseCode = "401", description = "Requesting context lacks authentication clearance."),
        @ApiResponse(responseCode = "44", description = "Target user record could not be found for the provided identifier.")
    })
    public ResponseEntity<RestResponse<UserDto>> updateUser(
        ...
        return ResponseEntity.ok(
                RestResponse.success("User changes successfully updated and saved.", updatedUser)
        );
    }
}